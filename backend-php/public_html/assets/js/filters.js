...
const lastButton = document.getElementById('lest_button');
...
function filterPrice(filter = null, subCat = null) {
  ...
  fetch(myFetchRequest, {
    ...
    .then(products => {
      let pages = pagesArray(products, 9);
      console.log(pages[0]);
    ...
    productCard.innerHTML = `
            <div class="bg-gray-100 rounded-2xl p-6 cursor-pointer hover:-translate-y-2 transition-all relative">
                ...
                <div id="${product.id}" class="w-2/3 h-[220px] overflow-hidden mx-auto aspect-w-16 aspect-h-8 article-image">
                    <img src="${product.url_image ? `${currentPageUrl}/assets/images/${product.url_image}` : `http://<?= $serverName ?>/assets/images/tea-coffee.png`}" alt="image ${product.name}" class="h-full w-full object-contain" />
                </div>
