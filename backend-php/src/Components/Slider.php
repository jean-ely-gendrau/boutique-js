    ...
    public function generateProductList(array $products, string $id)
    {
        ...
        foreach ($products as $productItem):
          ...
          <div class='product text-center mt-4' data-price='{$productItem->price}' data-id='{$productItem->id}' data-name='{$productItem->name}'>
            ...
            <button data-js='handlePost,click' data-define-request='http://{$_SERVER['HTTP_HOST']}/addtobasket/{$productItem->id}' type='button' class='add-to-cart w-full flex items-center justify-center gap-3 mt-6 px-4 py-2.5 bg-transparent hover:bg-gray-200 text-base text-[#333] border-2 font-semibold border-[#333] rounded-xl'>
