...
class Products
{
    ...
    public function produitLeak($categoryName, $pageURL)
    {
        ...
        foreach ($mostSell as $sellMost) {
            ...
            echo "</svg>";
            echo "</div>";
            echo '<img id="' . $sellMost['id_product'] . '" class="article-image" src="http://' . $this->serverPath . '/assets/images/' . $sellMost['images']['main'] . '"alt="' . $sellMost["name"] . '">';
            echo "<p id='" . $sellMost['id_product'] . "' class='mt-3 font-bold article-name'>" . $sellMost["name"] . "</p>";
        ...
        echo "<label for='counterSubCat'>" . $type . $pageURL . ": </label>";
        echo "<select name='counterSubCat' id='counterSubCat' onchange='checkSelection()'>";
        echo "<option value='99'>ici que ça se passe</option>";
        ...
        echo "</select>";
        echo "<button id='submitSubCat' type='submit' class='w-48 mt-4 px-4 py-3 bg-[#333] hover:bg-[#222] text-white rounded-full'>allons voir</button>";
        echo "<div id='text'></div>";
        echo "</form>";
        echo "</div>";

        if (isset($_POST['counterSubCat'])) {
            ...
            if ($counterSubCat === '0' || $counterSubCat === '1' || $counterSubCat === '2' || $counterSubCat === '3' || $counterSubCat === '4' || $counterSubCat === '5') {
                ...
                foreach ($subCat as $catSub) {
                    ...
                    echo "<div class='mx-auto flex justify-start max-w-6x1'>";
                    echo "<h2 class='bg-gray-100 ml-10 p-2 rounded-xl'>" . $catSub["description"] . "</h2>";
                    echo "</div>";
                    ...
                    foreach ($products as $product) {
                        ...
                        echo "</svg>";
                        echo "</div>";
                        echo '<img id="' . $product['id_product'] . '" class="article-image" src="http://' . $this->serverPath . '/assets/images/' . $product['images']['main'] . '"alt="' . $product["name"] . '"> <br>';
                        echo "<p id='" . $product['id_product'] . "' class='mt-3 font-bold article-name'>" . $product["name"] . "</p>";
                        echo "<div class='flex justify-center'>";
                        echo "<p class='mt-3 font-bold mr-2'>" . $product["price"] . "€</p>";
                        echo "<p class='mt-3 font-medium text-gray-300'>" . $product["price"] . "€";
                        echo "</div>";
                        echo "<div>";
                        echo '<button type="button" class="w-48 mt-4 px-4 py-3 bg-[#333] hover:bg-[#222] text-white rounded-full">Add to cart</button>';
                        echo '</div>';
