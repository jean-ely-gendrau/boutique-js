...
use App\Boutique\Interfaces\DebugInterface;
  ...
  public static function view(mixed $data)
  {
    ...
    '<div id="contain-debug" class="border-t-2 border-black fixed bottom-0 left-0 right-0 w-full z-50 flex-colum debug-min">
      <div class="flex flex-row bg-gray-700 justify-evenly w-50 m-auto p-2">
        <div class="rounded-full px-2 bg-white my-auto">DATA : <span class="text-yellow-700 font-medium">' . $count . '</span></div>
        <div class="rounded-full px-2 bg-white my-auto">POST : <span class="text-yellow-700 font-medium">' . count($_POST) . '</span></div>
        <div class="rounded-full px-2 bg-white my-auto">GET : <span class="text-yellow-700 font-medium"> ' . count($_GET) . '</span></div>
