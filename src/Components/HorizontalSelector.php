...
class HorizontalSelector
{
    ...
    public function generateProductList($product)
    {
        ?>
            ...
            <ul class="block list-none p-0 whitespace-nowrap overflow-hidden max-w-6xl px-[20px]">
                <?php foreach ($product->AllProduct() as $productItem): ?>
                    <div class="bg-gray-100 w-60 h-80 inline-block relative text-center m-2.5 rounded-xl">
                        ...
                        <img src="http://<?= $this->serverName ?>/assets/images/<?= $productItem[
    'images'
]['main'] ?>" alt="<?= $productItem[
    'name'
] ?>" class="w-32 h-28 mx-auto mt-12" />
                        <p class="mt-3 font-bold"><?= $productItem[
                            'name'
                        ] ?></p>
                        <div class="flex justify-center">
                            <p class="mt-3 font-bold mr-2"><?= $productItem[
                                'price'
                            ] ?>€</p>
