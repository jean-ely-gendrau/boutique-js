...
class ProductsAdminForms
{
  ...
  public static function ProductsForm(...$arguments)
  {
     ...
     */
    $nameButton = $arguments['update-product'] ?? 'validation-product';
    $anchorButton = isset($arguments['update-product']) ? 'Modifier produit' : 'Ajouter produit';

    $formBuilderProduct->setClassActionGroup('flex flex-wrap w-full justify-between');
    $formBuilderProduct->addElementAction('button', $nameButton, $nameButton, [
      'class' =>
    ...
    if (isset($arguments['update-product'])) {
      $formBuilderProduct->addElementAction('submit', 'delete-user', 'delete-user', [
        'class' =>
        ...
        'attributes' => ['data-js' => 'handlePost,click', 'data-route' => '/api/delete/' . $modelProducts->getId() . '']
      ]);
    }
